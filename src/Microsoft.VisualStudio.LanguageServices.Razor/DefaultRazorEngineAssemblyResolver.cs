...
namespace Microsoft.VisualStudio.LanguageServices.Razor
{
    ...
    {
        public async Task<IEnumerable<RazorEngineAssembly>> GetRazorEngineAssemblies(Project project)
        {
            var compilation = await project.GetCompilationAsync();
            return GetRazorCustomizationAssemblies(compilation);
        }

        private static IEnumerable<RazorEngineAssembly> GetRazorCustomizationAssemblies(Compilation compilation)
        {
            ...
            // The ReferencedAssemblyNames and References are the same length and in the same order.
            // You can't easily map from one property to another, so iterating them together is the
            // most sensible thing to do.
            var identities = compilation.ReferencedAssemblyNames.GetEnumerator();
            var references = compilation.References.GetEnumerator();
            ...
            while (identities.MoveNext() && references.MoveNext())
            {
                ...
                if (identities.Current.Name == RazorAssemblyName)
                {
                ...
                for (var i = 0; i < attributes.Length; i++)
                {
                        ...
                        StringComparison.Ordinal))
                    {
                        ...
                        if (AssemblyIdentity.TryParseDisplayName((string)attribute.ConstructorArguments[0].Value, out identity))
                        {
